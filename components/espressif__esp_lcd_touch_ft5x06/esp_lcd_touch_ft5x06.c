...
static bool esp_lcd_touch_ft5x06_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    *point_num = (tp->data.points > max_point_num ? max_point_num : tp->data.points);
    ...
    for (size_t i = 0; i < *point_num; i++)
    {
        ...
        y[i] = 480 - tp->data.coords[i].y;
        // y[i] = tp->config.y_max - tp->data.coords[i].y;
...
static esp_err_t touch_ft5x06_i2c_write(esp_lcd_touch_handle_t tp, uint8_t reg, uint8_t data)
{
    ...
    // return i2c_master_write_to_device(0, ESP_LCD_TOUCH_IO_I2C_FT5x06_ADDRESS, write_buf, sizeof(write_buf), 1000 / portTICK_PERIOD_MS);
    // *INDENT-ON*
}
...
static esp_err_t touch_ft5x06_i2c_read(esp_lcd_touch_handle_t tp, uint8_t reg, uint8_t *data, uint8_t len)
{
    ...
    // return i2c_master_receive(dev_handle, data_rd, DATA_LENGTH, -1);

    /* Read data */
    // return i2c_master_write_read_device(0, ESP_LCD_TOUCH_IO_I2C_FT5x06_ADDRESS, &reg, 1, data, len, 1000 / portTICK_PERIOD_MS);
}
